...
contract RoseBond is Ownable {
    ...
    uint256 public discountRateForWhite;
    ...
    function bond() public payable {
        require(totalBondAmount <= etherLimit, "Bond is finished!");
    ...
    function getRedeemAmount(address _who) public view returns (uint256) {
        ...
        uint256 debtPerTime = user.currentDebt.div(
            endTime.sub(user.lastRedeemTime)
        );
        uint256 redeemAmount = debtPerTime.mul(
            block.timestamp.sub(user.lastRedeemTime)
        );
        return redeemAmount;
    }
    ...
    function _swapNativeToLP(uint256 amount, address recipient)
        private
        returns (uint256)
    {
        address token0 = roseLp.token0();
    ...
    function _swapNativeForToken(uint256 value, address recipient)
        private
        returns (uint256)
    {
        address[] memory path;
    ...
    // Set discount for white list rate by only owner
    function setDiscountRateForWhite(uint256 _discountRateForWhite)
        public
        onlyOwner
    {
        require(_discountRateForWhite <= MAX_PERCENT, "Max is 100%");
        discountRateForWhite = _discountRateForWhite;
    }
    ...
    function setEtherLimit(uint256 _etherLimit) public onlyOwner {
        etherLimit = _etherLimit;
    }
